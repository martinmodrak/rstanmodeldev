#This is mostly a work in progress, still keeping some code from a specific project
...
prepare_kfold <- function(K, model_defs, base_data, seed = NULL) {
    ...
    models <- model_names %>% purrr::map(function(model_name) {
        rstan::rstan_options(auto_write = TRUE)
        rstan::stan_model(here::here("stan", sprintf("%s.stan", model_name)))
    }) %>% setNames(model_names)

    model_defs <- model_defs %>% dplyr::mutate(id = 1:(dplyr::n()))
    model_defs_kfold <- model_defs %>% dplyr::crossing(fold = 1:K)

    if (!is.null(seed)) {
        set.seed(seed)
    }

    folds <- loo::kfold_split_random(K, base_data$N)
    data_list <- list()
    ...
    if (!is.null(model_defs$adapt_delta)) {
        control_list <- map(model_defs_kfold$adapt_delta, function(x) {
            list(adapt_delta = x)
        })
    } else {
        control_list <- NULL
    ...
    list(base_data = base_data, data_list = data_list, control_list = list, model_defs = model_defs, model_defs_kfold = model_defs_kfold,
        models_list = models[as.character(model_defs_kfold$model_name)], models_unique = models)
...
run_kfold <- function(kfold_def, name_for_cache) {
    if (!requireNamespace("matrixStats", quietly = TRUE)) {
        stop("Package \"matrixStats\" needed for this function to work. Please install it.",
             call. = FALSE)
    }

    cache_dir <- here::here("local_temp_data", name_for_cache)
    if (!dir.exists(cache_dir)) {
        dir.create(cache_dir)
    }
    fits <- sampling_multi(kfold_def$models_list, kfold_def$data_list, control_per_item = kfold_def$control_list, cache_dir = cache_dir,
    ...
    res_names <- kfold_def$model_defs %>% unite(full_name, -id) %>% dplyr::pull(full_name)
    kfold_res <- kfold_log_lik %>% map(kfold) %>% rlang::set_names(res_names)

    list(fits = fits, log_lik = kfold_log_lik, kfold = kfold_res)
}
...
extract_holdout_ranks <- function(list_of_stanfits, list_of_holdout, base_data) {
    ...
    holdout_ranks_less <- sweep(all_genes_holdout_gen, MARGIN = c(2, 3), STATS = base_data$counts, FUN = "<") %>% apply(MARGIN = c(2,
        3), FUN = sum)

    holdout_ranks_equal <- sweep(all_genes_holdout_gen, MARGIN = c(2, 3), STATS = base_data$counts, FUN = "==") %>% apply(MARGIN = c(2,
        3), FUN = sum)

    # If there are equal values, sample the rank randomly over the equal values Using a trich with rounded uniform random
    # numbers to get that in a vectorized way
    holdout_ranks <- holdout_ranks_less + round((holdout_ranks_equal + 1) * array(runif(length(holdout_ranks_equal)), dim(holdout_ranks_equal)) -
        0.5)
...
extract_holdout_ranks_all <- function(kfold_def, kfold_res) {
    kfold_def$model_defs$id %>% map(function(id) {
        ...
        holdout_for_model <- lapply(kfold_def$data_list[indices], "[[", "holdout")
        extract_holdout_ranks(fits_for_model, holdout_for_model, kfold_def$base_data) %>% as.tibble() %>% rownames_to_column("sample") %>%
            gather("gene", "rank", -sample) %>% mutate(model = kfold_def$model_defs$model_name[kfold_def$model_defs$id == id])
    }) %>% do.call(rbind, args = .)
}
...
plot_holdout_ranks <- function(ranks, binwidth = 1, facet = ~model) {
    ...
    ranks %>% ggplot(aes(x = rank)) + geom_segment(aes(x = 0, y = mean, xend = 100, yend = mean), colour = "grey25") + geom_polygon(data = data.frame(x = c(-10,
        0, -10, 110, 100, 110, -10), y = c(lower, mean, upper, upper, mean, lower, lower)), aes(x = x, y = y), fill = "grey45",
        color = "grey25", alpha = 0.5) + geom_histogram(breaks = seq(1, 101, by = binwidth), closed = "left", fill = "#A25050",
        colour = "black") + facet_wrap(facet, scales = "free_y") + ggtitle("Posterior ranks of heldout observations")
...
extract_log_lik_K <- function(list_of_stanfits, list_of_holdout, ...) {
    ...
    # `log_lik_heldout` will include the loglike of all the held out data of all the folds.  We define `log_lik_heldout` as a
    # (samples x N_obs) matrix (similar to each log_lik matrix)
    log_lik_heldout <- list_of_log_liks[[1]] * NA
    for (k in 1:K) {
        log_lik <- list_of_log_liks[[k]]
        samples <- dim(log_lik)[1]
        N_obs <- dim(log_lik)[2]
        # This is a matrix with the same size as log_lik_heldout with 1 if the data was held out in the fold k
...
kfold <- function(log_lik_heldout) {
    ...
    log_col_means_exp <- function(x) {
        # should be more stable than log(colMeans(exp(x)))
        S <- nrow(x)
        matrixStats::colLogSumExps(x) - log(S)
    }
    # See equation (20) of @VehtariEtAl2016
    pointwise <- matrix(log_col_means_exp(log_lik_heldout), ncol = 1)
    ...
    out <- list(pointwise = pointwise, estimates = estimates)

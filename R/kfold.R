prepare_kfold <- function(K, model_defs, base_data, seed = NULL) {
    ...
    models = foreach(model_name = model_names) %do% {
    ...
    list(base_data = base_data, data_list = data_list, control_list = list, model_defs = model_defs, model_defs_kfold = model_defs_kfold, models_list = models[as.character(model_defs_kfold$model_name)], 
...
run_kfold <- function(kfold_def, name_for_cache) {
    ...
    fits = sampling_multi(kfold_def$models_list, kfold_def$data_list, control_per_item = kfold_def$control_list, cache_dir = cache_dir, cores = getOption("mc.cores"))
    ...
    kfold_res <- kfold_log_lik %>% map(kfold) %>% set_names(res_names)
...
extract_holdout_ranks <- function(list_of_stanfits, list_of_holdout, base_data) {
    ...
    holdout_ranks_less <- sweep(all_genes_holdout_gen, MARGIN = c(2, 3), STATS = base_data$counts, FUN = "<") %>% apply(MARGIN = c(2, 3), FUN = sum)
    
    holdout_ranks_equal <- sweep(all_genes_holdout_gen, MARGIN = c(2, 3), STATS = base_data$counts, FUN = "==") %>% apply(MARGIN = c(2, 3), FUN = sum)
    
    # If there are equal values, sample the rank randomly over the equal values Using a trich with rounded uniform random numbers to get that in a
    # vectorized way
    holdout_ranks = holdout_ranks_less + round((holdout_ranks_equal + 1) * array(runif(length(holdout_ranks_equal)), dim(holdout_ranks_equal)) - 0.5)
...
extract_holdout_ranks_all <- function(kfold_def, kfold_res) {
    kfold_def$model_defs$id %>% map(function(id) {
        ...
        holdout_for_model <- lapply(kfold_def$data_list[indices], "[[", "holdout")
        extract_holdout_ranks(fits_for_model, holdout_for_model, kfold_def$base_data) %>% as.tibble() %>% rownames_to_column("sample") %>% gather("gene", 
            "rank", -sample) %>% mutate(model = kfold_def$model_defs$model_name[kfold_def$model_defs$id == id])
    }) %>% do.call(rbind, args = .)
}
...
plot_holdout_ranks <- function(ranks, binwidth = 1, facet = ~model) {
    ...
    ranks %>% ggplot(aes(x = rank)) + geom_segment(aes(x = 0, y = mean, xend = 100, yend = mean), colour = "grey25") + geom_polygon(data = data.frame(x = c(-10, 
        0, -10, 110, 100, 110, -10), y = c(lower, mean, upper, upper, mean, lower, lower)), aes(x = x, y = y), fill = "grey45", color = "grey25", alpha = 0.5) + 
        geom_histogram(breaks = seq(1, 101, by = binwidth), closed = "left", fill = "#A25050", colour = "black") + facet_wrap(facet, scales = "free_y") + 
        ggtitle("Posterior ranks of heldout observations")
    
}

# Functions extract_log_lik and kfold taken from https://github.com/stan-dev/stancon_talks/blob/master/2017/Contributed-Talks/07_nicenboim/kfold.Rmd
...
extract_log_lik_K <- function(list_of_stanfits, list_of_holdout, ...) {
    ...
    # `log_lik_heldout` will include the loglike of all the held out data of all the folds.  We define `log_lik_heldout` as a (samples x N_obs) matrix
